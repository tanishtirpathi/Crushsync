...
const Result = forwardRef((props, ref) => {
  ...
  return (
    <div className="flex flex-col items-center w-screen min-h-screen text-white font-[syne] px-4 py-5 space-y-12">
      <h1 className="text-s text-[#ffe23f] font-bold">
        Take your guts and see the result{" "}
      </h1>
      ...
      <div className="bg-[#1E2B57] px-8 py-6 rounded-2xl border border-[#FF3B3B] text-center max-w-xl">
        <h2 className="text-[#FFE23F] text-xl">
          Compatibility Score: <b>{data.score}</b>/100 — Start your code love
          story!
        </h2>
      </div>
      ...
      <div className="flex flex-col items-center justify-center gap-10 w-full">
        <div className="bg-[#1E2B57] p-6 rounded-2xl shadow-xl">
          <FollowerComparisonChart user1={data.user1} user2={data.user2} />
        </div>
        <div
          id="compare-section"
          className="bg-[#1E2B57] rounded-md flex justify-center items-center gap-x-4 px-10 py-5"
        >
      <div  className="w-80 text-center">
         ...
         <div>
           <img src="./another.jpg" className="rounded-md shadow w-90 " alt="" />
         </div>
        </div>
      </div>

      {/* Footer */}
      <h3 className="text-2xl text-[#FF3B3B] shadown-md italic pt-4">
        {scoreMessage}
      </h3>
    </div>
  );
